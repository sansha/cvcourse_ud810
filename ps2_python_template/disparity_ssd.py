...
def disparity_ssd(L, R):
    ...
    for lineIdx in range(L.shape[0]):
        ...
        for colIdx in range(L.shape[1]):
            ...
            _, _, min_loc, _ = cv2.minMaxLoc(sq_diff)
            if visualization:
                ...
                x_val = dist[min_loc[0]] + colLeftMin
                disp_selected_patch[:, x_val:x_val + offset * 2 + 1] = right_slice[:, min_loc[0]:min_loc[0] + patch_left.shape[1]]

                normalizedD = cv2.normalize(D[0:lineMin + 1, :], None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8U)
                ...
                cv2.waitKey(1)
            D[lineIdx - offset][colIdx - offset] = dist[min_loc[0]]
